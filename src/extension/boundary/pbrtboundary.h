...
#ifndef PBRT_BOUNDARY_H
#define PBRT_BOUNDARY_H
...
struct PBRTBoundary :Boundary<3> {
    PBRTBoundary(
        function<double(arrayd<3> const &)> cond,
        std::shared_ptr<Primitive> boundary,
        double &maxError) : Boundary(cond, maxError), boundary(boundary){
		auto mesh = dynamic_pointer_cast<Triangle>(
			dynamic_pointer_cast<GeometricPrimitive>(dynamic_pointer_cast<BVHAccel>(boundary)->primitives[0])->shape)->mesh;
		std::vector<std::array<double, 3>> vertices;
		std::vector<std::array<int, 3>> triangles;
		for (int i = 0; i < mesh->nVertices; i++) {
			vertices.push_back({ mesh->p[i].x,mesh->p[i].y, mesh->p[i].z });
		}
		for (int i = 0; i < mesh->nTriangles; i++) {
			triangles.push_back({ mesh->vertexIndices[3*i],mesh->vertexIndices[3*i+1],mesh->vertexIndices[3*i+2]});
		}
		distance = tmd::TriangleMeshDistance(vertices, triangles);
	};
    double intersectRay(arrayd<3> &o, arrayd<3> &d, arrayd<3> &n) const override;
    double minDistFromBoundary(arrayd<3> &x) const override;
    double minDistFromBoundary(arrayd<3> &x, arrayd<3> &d) const override;
private:
    std::shared_ptr<Primitive> boundary;
